    ...
    @foreach ($mainMenus as $mainMenu)
        @if ($mainMenu->is_single)
            <li class="nav-item d-sm-none d-lg-block">
                <a class="nav-link text-uppercase text-truncate" href="{{ route('lojaprodutos', ['main' => $mainMenu->slug]) }}">
                    {{ $mainMenu->name }}
                </a>
            </li>
        @else
            <li class="nav-item dropdown d-sm-none d-lg-block">
                ...
                <ul class="dropdown-menu" aria-labelledby="{{ $mainMenu->slug }}">
                    ...
                    <li>                        
                        <a class="dropdown-item text-truncate text-uppercase" href="{{ route('lojaprodutos', ['main' => $mainMenu->slug, 'sub' => $submenu->slug]) }}">
                            {{ $submenu->name }}
                        </a>
                    </li>
                    @endforeach
...
<div class="d-block d-md-none" style="width: 100%">
    ...
    @foreach ($mainMenus as $mainMenu)
        @if ($mainMenu->is_single)
            <li class="nav-item d-sm-none d-lg-block">
                <a class="nav-link text-uppercase text-truncate" href="{{ route('lojaprodutos', ['main' => $mainMenu->slug]) }}">{{ $mainMenu->name }}</a>
            </li>
        @else
            <li class="nav-item dropdown d-sm-none d-lg-block">
