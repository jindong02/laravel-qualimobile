                    ...
                    <div class="table-toolbar">
                        <div class="hb-flex-between">
                            <div class="hb-flex-list">
                                <a href="{{ route('admin.lojasubmenus.create') }}" class="hb-button btn-success btn-sm">
                        ...
                        <div id="detailedFilterForm" style="display: none" class="mt-3">
                            @include('admin.lojasubmenus.partials.filter')
                        </div>
                    </div>
                    <table class="table table-bordered sorted_table mt-3" style="position: relative">
                        ...
                        @if ($submenus->count())
                            ...
                            <tr class="draggable-element" data-id="<?= $submenu->id; ?>">
                                ...
                                <td><a href="{{ route('admin.lojamenus.edit', $submenu->mainmenu?$submenu->mainmenu:"") }}" class="link-primary">{{ $submenu->mainmenu?$submenu->mainmenu->name:"" }}</a></td>
                                <td>
                                    <a class="btn btn-primary btn-sm" href="{{ route('admin.lojasubmenus.edit', $submenu) }}">EDITAR</a>
                                    <a class="btn btn-danger btn-sm" href="{{ route('admin.lojasubmenus.destroy', $submenu) }}" data-toggle="modal" data-target="#deleteConfirmModal" data-submenu="{{ $submenu->name }}">EXCLUIR</a>
                                </td>
                            </tr>
                            @endforeach
                        @else
...
@include('admin/lojasubmenus/partials/delete_confirm')
...
<script>
    ...
    function rearrange(targetRowID, prevRowID, nextRowID) {
        $.get(`/admin/lojasubmenus/rearrange?target_row=${targetRowID}&prev_row=${prevRowID}&next_row=${nextRowID}&mainmenu_id=<?= request()->get('menu_principal') ?>`, function(data) {
