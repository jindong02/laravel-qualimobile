                ...
                <div class="card_content">
                    <div class="table-toolbar">
                        <div class="hb-flex-between">
                            ...
                            <div class="hb-flex-list">
                                <a href="{{ route('admin.menus.create') }}" class="hb-button btn-success btn-sm">
                            ...
                            <tr class="draggable-element" data-id="<?= $menu->id; ?>">
                                ...
                                <td>
                                    @if ($menu->is_single)
                                        Menu único
                                    @else
                                        <a href="{{ route('admin.submenus.index') }}?menu_principal={{ $menu->id }}" class="link-primary">{{ $menu->submenus->count() }}</a>
                                    @endif
                                </td>
                                <td>
                                    <a class="btn btn-primary btn-sm" href="{{ route('admin.menus.edit', $menu->id) }}">EDITAR</a>
                                    <a class="btn btn-danger btn-sm" href="{{ route('admin.menus.destroy', $menu) }}" data-toggle="modal" data-target="#deleteConfirmModal" data-menu="{{ $menu->name }}">EXCLUIR</a>
                                </td>
                            </tr>
                            @endforeach
                        @else
...
@include('admin/menus/partials/delete_confirm')
...
<script>
    ...
    function rearrange(targetRowID, prevRowID, nextRowID) {
        $.get(`/admin/menus/rearrange?target_row=${targetRowID}&prev_row=${prevRowID}&next_row=${nextRowID}`, function(data) {
